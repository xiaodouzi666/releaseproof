  ...
  return (
    <section className="intake-section" id="request-intake" aria-labelledby="intake-heading">
      <div className="section-heading section-heading--split">
        <div>
          <span className="eyebrow"><ScanLine size={14} /> Request intake</span>
          <h2 id="intake-heading">Give the agent a messy request.</h2>
        </div>
        <p>Natural language or a ticket screenshot. The control plane validates every proposed write.</p>
      </div>

      <div className="scenario-block">
        <div className="field-label-row">
          <label id="scenario-label">Choose a walkthrough</label>
          <span>Optional · loads a safe demo request</span>
        </div>
        {scenariosLoading ? (
          ...
          <div className="inline-notice inline-notice--warning"><AlertTriangle size={16} /> {scenariosError}</div>
        ) : scenarios.length ? (
            ...
            {scenarios.map((scenario, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="scenario-tag">{scenario.riskHint || scenario.tag || "guided"}</span>
                </button>
              );
            })}
          </div>
        ) : (
          <div className="inline-notice">No preset scenarios returned. You can still enter a request below.</div>
        )}
      </div>
      ...
      <div className="intake-grid">
        <div className="request-editor">
          <div className="field-label-row">
            <label htmlFor="request-text">Access request</label>
          ...
          <textarea
            ...
            placeholder="Example: Nina needs production billing access until Friday to investigate INC-4821. Give only the actions required for log inspection."
            maxLength={6000}
            rows={9}
          />
          <div className="editor-footnote">
            <Sparkles size={14} aria-hidden="true" />
            Qwen extracts intent; deterministic policy remains authoritative.
          </div>
        </div>

        <div className="image-uploader">
          <div className="field-label-row">
            <label htmlFor="ticket-image">Ticket screenshot</label>
          ...
          {imageDataUrl ? (
            <div className="upload-preview">
              <img src={imageDataUrl} alt={`Preview of ${imageName || "uploaded ticket"}`} />
            ...
            <div className="upload-empty-state">
              <button type="button" className="upload-dropzone" onClick={() => fileRef.current?.click()}>
                ...
                <small>Live Qwen vision can read the original ticket without a separate OCR service.</small>
              ...
              <button type="button" className="sample-image-button" data-testid="load-sample-image" onClick={onLoadSample}>
                <FileImage size={14} /> Load adversarial ticket sample
              </button>
              <p className="sample-disclosure">Recorded Demo does not inspect image pixels; this button also fills matching text for a deterministic safety run. Live Qwen vision reads the image.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="intake-actions">
        <div className="privacy-note">
          ...
          Demo data only. Never paste production secrets or credentials.
        </div>
        <button className="button button--primary button--large" data-testid="run-workflow" type="button" disabled={!canSubmit} onClick={onSubmit}>
          {submitting ? <><LoaderCircle className="spin" size={18} /> Starting analysis</> : <>Start guarded analysis <ArrowRight size={18} /></>}
        </button>
      </div>
    </section>
  );
